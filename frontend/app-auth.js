...
const TOKEN_KEY = "marketsim_token";
...
function showAuth() {
  ...
  document.dispatchEvent(new CustomEvent("marketsim:logout"));
  setStatus("未登录");
...
function showApp(user) {
  ...
  els.userBadge.textContent = `账号：${user.username}`;
  document.querySelector("#bottomNav")?.classList.remove("hidden");
  document.dispatchEvent(new CustomEvent("marketsim:login", { detail: { user } }));
  clearInterval(state.refreshTimer);
...
els.logoutBtn.addEventListener("click", async () => {
  try {
    ...
    // Local logout should proceed even if the server session already expired.
  }
  showAuth();
  showToast("已退出登录");
});
